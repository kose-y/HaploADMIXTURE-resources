...
function main()
	...
	I = 940
        ...
        T = Float64
        T2 = Float32
        g = SnpLinAlg{T2}(SnpArray("/mnt/Data2/HGDP/HGDP_940_even.bed"))#SnpArrays.datadir("EUR_subset.bed"))")))#"/home/kose/SKFR/1000genomes_new/SCOPE/misc/simulations/n1k_10k_5pops_final.bed"))#SnpArrays.datadir("EUR_subset.bed")));
        rng = StableRNG(seed)
        Q = 3
...
#     d_cu, g_cu = nothing, nothing
        ...
        q = copy(d.q)
        p = copy(d.p)
        writedlm("HGDP_940_$(2 * nblock)_K$(K)_$(seed)_haplo_q.txt", q)
        writedlm("HGDP_940_$(2 * nblock)_K$(K)_$(seed)_haplo_p.txt", p)
    end
    end
    end
end
